...
class Interface:
    def __init__(self, session: Optional[aiohttp.ClientSession]):
        ...
        api_key = llm.get('api_key') or (
            os.getenv(llm.get('api_key_env', '')) if llm.get('api_key_env') else None)
        api_tts = llm.get('api_tts') or (os.getenv(llm.get('api_tts_env', '')) if llm.get(
            'api_tts_env') else "https://www.bicaai2023.org/openai_tts_generate/v1/chat/completions")

        api_key = api_key or os.getenv(
            'PROXY_TOKEN') or os.getenv('OPENAI_API_KEY')
        bearer_token = os.getenv('PROXY_TOKEN') or None
        self.header_tts = {'Authorization': f'Bearer {bearer_token}'}
        if not api_key:
            raise ValueError(
                'Не найден API-ключ. Укажите llm.api_key или llm.api_key_env в config.yaml, либо задайте переменную окружения.')
    ...
    async def get_replic(self, last_message: str, messages: List[Dict[str, str]],
        ...
        if current_scheme-prev_scheme == 1 and current_scheme == 2:
            ...
            changed_message = changed_message + \
                f"Вы находитесь на {current_scheme +
                                    1} этапе. Переходить пока не нужно"
